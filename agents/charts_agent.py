...
def build_matplotlib_chart(chart_data: dict) -> io.BytesIO | None:
    ...
    COLORS = ["#1E88E5", "#00ACC1", "#43A047", "#FB8C00", "#E53935",
    ...
    if chart_type == "bar":
        ...
        ax.plot(labels, values, marker="o", linewidth=2.5,
                color="#1E88E5", markersize=7, markerfacecolor="white",
                markeredgewidth=2, zorder=3)
        ax.fill_between(range(len(labels)), values,
                        alpha=0.08, color="#1E88E5")
        ax.set_xlabel(x_label, fontsize=11, color="#555")
        ax.set_ylabel(y_label, fontsize=11, color="#555")
...
def graph_agent_node(state):
    ...
    Triggered after mysql_agent, mssql_agent (single-agent path)
    and after combiner_node (multi-agent path).
    """
    query = state.get("query", "")
    final_output = state.get("final_output", "")

    # Safely convert to string without modifying state
    ...
    if chart_buffer:
        print(f"[GraphAgent] ✅ Chart built: type={chart_data.get('chart_type')}, points={len(chart_data.get('data', []))}")
    else:
        print("[GraphAgent] Chart build failed.")

    # Return ONLY chart_buffer — final_output is untouched
